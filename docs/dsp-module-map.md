...
⏭️ **`api.ts` 剩余 ≈30%**：主要是 **weixin 的四个 POST 动作**（login-start ／ remove-account ／ save-routes ／ set-enabled）与 **session-cleanup 的 POST**（**会真删会话 ⇒ 需要"隔离 DSH_HOME"才敢测**，属**下一步要设计夹具**的那种，不是直接补）。，或按 §5.1「更要有语义断言」处理下表 #10 的接线钉。<br>⚠️ **上列百分比是"某次 coverage 跑的实测读数"**（锚定 2026-09-25 ⑤ 开工时那一跑），**不是"当前值"**（每次补测都会变）⇒ 引用时须标时点（§4.2-㉞ 同族）。
