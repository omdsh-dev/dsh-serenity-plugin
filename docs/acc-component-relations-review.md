...
- **状态**：讨论稿 v0.1 ⇒ **已实施**（**2026-09-15 同日落地**）：候选 **C1 / C2 / C3 / C4 / C5 / C6 全部实施**（**C7 判定"不动"**，同本文档）。提交 `35a6088`（插件仓，121 文件 / +7438 / −2217）；**逐步记账 + 每步我方独立复核**见 SESSION S142 **§12.15~§12.21**。
  - ⚠️ **本文正文仍是"改前"的现状描述**，作为**讨论留档**不再逐段更新。要读**改后**的现状，看 SESSION.md §12 与插件仓源码。
  - ⚠️ **本文两处估算已被实测修正，勿再沿用**：① **C5 的"10 个渲染点"里实际只有 1 处是真重复**（其余是同一判据的多个渲染点，按设计**本就不该合并**）；② **C6b 的"省 ~160 行"实测只省 99 行**（P2"不合并循环"的固有代价）。
  - ⚠️ **尚未发布 / deploy ⇒ 运行态仍是 v1.34.0**（D14 待令）。
- **日期**：2026-09-15（S142）
...
> **📌 事实底稿（2026-09-15 补）**：候选 C2 / C4 / C5 / C6 与 Q9 的**现状事实**（逐处 `file:line`、数量、补偿代价、重叠矩阵、方法局限）已独立取证并汇编为 **`acc-component-relations-current-state.md`** —— 本节与全文**只保留候选与取舍**，**事实一律指向该文件**（单向引用，避免双真相源）。该文件同时给出**三条对本稿估计的更正**（C2 不是 6 套而是 19+1；C4 不是 4 面而是 5 面；C5 不是 4 处而是 10 个渲染点）。
...
### 5.1 ✅ 裁定结果（2026-09-15，**九条全闭**）

> 事实底稿见 `acc-component-relations-current-state.md`；逐条推理与备选见 SESSION S142 §12。

| # | 裁定 | 备注 |
|---|---|---|
| **Q1** | **同意**——判据「能落 harness 原语就不自造协议 + 脚本管道」升为**明面判据** | 本轮后续裁决即按它判 |
| **Q2** | **同意**——**废除 SEP**（`session-tool` 脚本钩子） | 本 CCC 实测零 adoption |
| **Q3** | 注入形态取 **a2**（绑定期间 `systemPrompt.section`） | 抗压缩 |
| **Q4** | 声明放 **(a) `SESSION.md` frontmatter** | ⚠️ 残余风险 = compaction 会重写 SESSION.md ⇒ **需补护栏**：keeper 文案加"保留 frontmatter 原样" |
| **Q5** | **C2 定**：**不新增名单** → **改用 DSH 已有持久来源**（`discoverCccs` 三层）+ **短路→取并集**；范围含 **L1 + L2** | ⚠️ **推翻了讨论稿的原备选**（"新增 CCC 注册表"）——所有者追问「dsh 读不到历史工作区吗」后实测：**已有且已接好**（`/serenity/cccs` + 设置面板） |
| **Q6** | **自动消解**——不新增名单 ⇒ 落点问题不存在 | — |
| **Q7** | **C5 + C4 都做** | ⚠️ 二者**均已无用户可感知部分**（C5 的面板显示被所有者答"不需要"；C4 的原最大好处"删 Skiff 93 行重试"已归 C2）⇒ 二者收益均为**整洁度** |
| **Q8** | **C6 定**：前提 **D59 不破**（所有者「仍然唯一」）｜**脚本侧拆两半、退掉独立脚本**（机制那半进插件、排障那半留开发面）｜**引擎取 P2**（共用时钟代码、各持定时器） | 见 SESSION §12.8 + 12.8-② |
| **Q9** | **已执行**——零引用导出扫描（产物 84 KB） | 结论：真孤儿 **20 条**；~190 条 DEAD 属"自用型"（只该删 `export`） |

**顺带修掉一项**（取证撞出，不属任何候选）：脚本侧 `✅` 假满足缺陷——`diagCcc` 判决行只统计 `✗` 而漏掉 `⏸` ⇒ **每天北京 8~18 点报"全满足"而实际不唤起**。已修（抽出 `judgeWake()` 单一判据 + 判决分「`✗` 配置问题 / `⏸` 等待中」）+ **实测复现前后对比**，specs 镜像同批。
