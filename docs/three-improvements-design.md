...
| # | 需求 | 用户裁决 |
|---|------|---------|
| ① | ACC 核心提示词 metaphor 调整 | **海船→星舰意象升级**（三层 SHIP/VOYAGE/CREW + 约束映射 M-1~M-4 不变）；Harbor Inspection → **Departure Inspection** |
| ② | Serenity 配置开关与详设合一 | 改**行内展开详设**——整行点击展开，重型编辑器（微信桥/外部访问）也内联 |
| ③ | 可开关自动删除指定日期前 dsh 会话 | 目标 = **DSH 平台会话（对话历史）**；lastActive 基准 + 手动按钮 + 不过滤不归档**直接物理删**（live 保护保留） |

> **状态（2026-09-05）**：方案 v0.1 定稿（决策点全部拍板）。**未实现**——实施顺序 ② → ① → ③，bump 合成一版发布（D14 等用户显式要求）。
...
### 1.4 拍板（2026-09-05 用户裁决）

- **词表**：采用 §1.2 推荐表；sea→deep space / anchor→dock clamp / set sail→launch
- **Harbor Inspection 改名**：→ **Departure Inspection**（启航检查——前者）
- 保留 Ship of Theseus 原词（跨意象成立）
- specs 仓同步：待 dsp 侧文本定稿后再评估（含 sea 意象处同步）
...
### 2.3 拍板（2026-09-05 用户裁决）

- **展开触发**：整行点击（非行尾按钮）——点击行 toggle 展开态
- **重型编辑器内联**：微信桥 / 外部访问（AccountsEditor）/ Autopilot 状态 / PublicAsk 等**全部改挂在所属开关行下内联展开**（不保留独立折叠区）；行内滚动承载复杂编辑器
- help 浮层内容并入 detail 面板顶部（不再 hover 才能见）
...
### 3.3 拍板（2026-09-05 用户裁决）

- **阈值基准**：lastActive（最后活动）——删除最后活动早于 N 天的会话（header 时间或日志最后事件）
- **触发**：手动按钮（面板「清理旧会话」按钮 + 可选 MSM 手动触发）；不做定时自动
- **不过滤**：不要求 completed/关闭态——任何非 live 会话到期即删
- **不归档隐藏、直接物理删**：不做 archiveSession 先行隐藏、不做回收站过渡——rm 直接删文件
- **安全底线（保留）**：live 会话保护跳过（删正在运行的会话 = 灾难，不可妥协）

### 3.4 补充实现细节（拍板后定稿）

- **lastActive 判定**：读 session.jsonl(.zstd) 最后一行/最后事件时间戳；退化为文件 mtime（日志只追加，mtime ≈ 最后写）
- **阈值输入**：面板「保留天数」数字输入（如默认 30 天 = 删 30 天前最后活动的会话）
- **删除动作**：`rm -rf <session-dir>`；dryRun 预览列表（面板先显示将删 N 个 → 确认后执行——物理删不可恢复）
- **按钮位置**：settings 面板新增「会话清理」区块（需求②内联在「会话管理」开关行下）
