...
世界层  The World — one ship, one sea（宇宙设定：容器=船，认知=航行）
  ...
  │    ├─ 4. Harbor Inspection   → First Anchor（启动锚定）
  │    ├─ 5. The Logbook         → Session Tracking（持续记录）
  │    └─ 6. The Ship of Theseus → Continuity（CCE 1，跨时间身份）
  └─ 协作层 THE CREW（多 agent）
       ├─ 7. Crew Rotation       → Multi-Agent Cognition（CCE 5）
       └─ 8. Blueprint over Statue → Reconstruction > Preservation（CCE 4）
```
...
| 关系 | 定义 | 实例 |
|------|------|------|
| **部分-整体**（containment） | 上层宇宙包含下层实体 | World ⊃ Ship ⊃ {Hull, Deck, Drawings}；Voyage ⊃ {Inspection, Logbook, Theseus}；Crew ⊃ {Rotation, Blueprint} |
| **映射**（mapping） | 隐喻 ↔ 协议约束一一对应 | Hull ↔ Bounded Space；Deck Order ↔ Entropy；Drawings ↔ EAP；Harbor ↔ First Anchor；Logbook ↔ Session；Theseus ↔ Continuity；Rotation ↔ Multi-Agent；Blueprint ↔ Reconstruction |
...
| # | 约束 | 判据 |
|---|------|------|
| **M-1** | 每条隐喻必须映射一个协议约束 | 无映射的隐喻 = 修辞装饰 = 不允许 |
