...
## 8. 实施实测回填（**S9 后补齐 · 2026-09-19**）

> **取证时机**：2026-09-19 22:1x（运行态 = **v1.42.0**，宿主 0.1.5-rc.2）。
> 判据 = **仓库实际状态 + 运行态实测**，不采信当时的过程自述。

- **发布版本**: 🔴 **v1.35.0**（`2026-09-15`；S1~S7 全段一次到位，所有者令「实施 + 发版一次到位」）。
  ⚠️ **本 doc 的 §8 迟到 4 天回填**——期间已跨 v1.36~v1.42 **七次发布**（回填欠账见 S142 §8 B4）。

| 项 | 判据（§5 原定） | 实测结果 |
|---|---|---|
| **门禁四项** | typecheck / test / build / pack-check 全绿 | ✅ 全绿（v1.42.0 本轮复跑：typecheck 双面 ｜ **98 files / 1449 tests** ｜ build 200080 B ｜ pack-check 112 文件） |
| **S2 判据** | `grep -rn 'autopilot' src/` **仅剩白名单** | ✅ **77 处命中，逐处核对全属白名单**：注释（退场说明 / 历史沿革）、配置兼容键（`autopilotTrajectory` 旧键回退、`trajectory.autopilot` 段保留）、JSDoc 里的对照说明。**无一处是可执行机制** |
| **模块删除** | 4 个 src 模块消失 | ✅ **`glob **/autopilot*` 在两个层级均 0 命中**（src/ 与 hooks/ 全树）⇒ 模块**彻底不存在**，非"改名藏起来" |
| **S8 · `container_admin` 无 autopilot 域** | 调用该域应被拒 | ✅ **实测**：`domain:"autopilot"` 返回 `"domain" must be one of ["role","msm","config"]` ⇒ **域确实已不存在**（响亮拒绝，非静默 no-op） |
| **S8 · `acc-diag` 段数** | 由四降为**三**（① / ①b / ③） | ✅ **实测三段**：① live 运行态 ｜ ①b 唤醒调度器时钟 ｜ ③ 唤醒注册表。**② 面板解析与 ④ 唤起条件链确实不在**（§1 二阶裁定的预期结果） |
| **S8 · `hostContract`** | `checked` 数下降但 **`issues: []`** | ✅ **`ok: true, checked: 42, issues: []`**（⚠️ **判据是 `issues: []`，不是数字**——见 §6 风险表） |
| **S8 · 链路基座未伤** | **实调一次投递动作成功** | ✅ **强证据**：本轮 S142 全程用该链路自排唤醒并**多次真实收到**（`w-20260919-2211-f6fb` 22:13 到点投递 + 本轨迹 00:11 续接在办）⇒ **链路活跃可用** |
| **S9 · `trajectory.autopilot` 配置段保留** | CCC 巡航不得停摆 | ✅ **实测仍在**（`dashboard health` 配置面可见：`enabled/intervalHours/biasProvider/topPrompt(指针)/session/avoidWakeHours`）；S151 侧**仍在正常跑轮**（其会话标题实测 = 「轮117巡检与队列推送」） |
| **S9 · skiff 白名单** | 预期无需改 | ✅ 无角色点名 autopilot 域（现白名单实测为 `msm`/`tools` 面，不含该域） |
| **D59 废止 / D58 保留** | §7 的裁定 | ✅ **D59 已废止**（S142 §5 决策表已标）；**D58 保留且成为唯一轨迹调度机制**——本版（v1.42.0 §0Q）正是它的延续 |

### 🔴 三处**诚实标注**（不粉饰）

1. **④ 唤起条件链缺口仍然存在**（§1 登记的那条）：**「为什么这轮没唤起」在 CCC 自管理链上依然不可见**——
   🔴 **而且 2026-09-19 它真的发作了**：S151 的链断 **1 天 14 小时**（宿主未运行 ⇒ 超窗 ⇒ 判 missed 清除），
   **正是"不可见"的实证**——最后由**所有者追问**才发现，而非任何诊断面报出。
   ⇒ **本条本 doc 当时判为"代价 = 一条新条件链"**；实测表明**真实代价更高**：缺的不只是条件链，而是**链外的存活检查**。
   （方向已由 S151 呈给所有者，**S142 挂靠等结论**，不另建。）
2. **`hostContract.checked` 从"四段时代"到现在的 42**：本 doc §6 预判"数字会变小"——
   ⚠️ **但 42 这个数字无法与当时直接对比**（期间经历 v1.36~v1.42 多次契约面增删，如 `storageDomain` 新增）。
   ⇒ **只报事实："issues: []" 成立**；**不编造"少了几个"的差值**（那需要当时的快照，本 doc 未存）。
3. **回填迟到**：§8 原写"待 S8 后写"，实际拖到 **v1.42.0**（4 天 / 7 个版本）。
   🔴 **这本身是一类欠账**——"待回填"的段落**没有触发机制**，于是它静默留在那里。
   ⇒ 与 S142 §7.2 那条教训同源：**"未结项清单会腐烂"，包括文档里的"待填"格**。

---

## 9. 一句话结论（回填后）

> **退场干净、无残留、基座未伤**：机制模块全树 0 命中、域响亮拒绝、`hostContract issues: []`、链路真实在用。
> **唯一真实代价 = ④ 缺口**，且 2026-09-19 已由 S151 的断链事故证明它**比当时评估的更贵**。
