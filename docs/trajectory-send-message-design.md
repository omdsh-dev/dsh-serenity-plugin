...
> 🔴 **本文部分被取代（2026-09-17，S142 D66）——读本文前先读这段**
>
> 所有者 2026-09-17 令：「希望 send message 时**不排队**，可以直接作为**提示词即时注入**进去」
> ⇒ **§12.B / §4 等处"在跑会话排在轮次边界 / 不打断当前轮"的结论已作废**。
>
> | 项 | 本文（设计时） | 现行（v1.39.2 起） |
> |---|---|---|
> | 目标在跑轮次 | `followup` ⇒ 排在**轮次边界**（不打断当前轮） | **`steer` ⇒ 当场注入当前轮**（`agent.status !== 'idle'` 时） |
> | 目标空闲 | `followup` | `followup`（宿主同步置 running ⇒ **立即起轮**）——**不变** |
> | `steer` 抛错 | — | **退回 `followup`**（fail-safe：宁可排队，不可静默丢消息） |
> | `send-later` | `followup` | **`followup`，一行未动** |
>
> **本文保留为设计沿革**（R↓：蓝图重于此雕像）——它是当时判据的出处，不是现行行为的描述。
> 现行语义的**唯一真相源**依次为：① 工具描述（`src/tools/trajectory.ts`）② 实现与其函数头注释
> （`src/wake-scheduler.ts` 的 `sendToTrajectory`）③ 会话记录 S142 §0r。
> ⚠️ **动作已更名**：`send-message` → **`send-now`**（2026-09-17，硬切无别名）。
> ⚠️ **本文 §12.B 的实测延迟数值（6.2s~31s 冷载入）仍然有效**——「实时」到不了"立刻执行"这一条**没变**。
>
> **状态**：**设计稿 v0.2 —— 三个决策点已由所有者裁决（2026-09-16），设计**已确认**；实现**待令**（D14）。**
...
> | # | 问题 | 裁决 |
> |---|---|---|
> | Q1 | 「回应」= 单向投递 还是 请求-响应 | **单向投递**（原话：「**投递即可，不用管回应**」） |
> | Q2 | 防乒乓节流 | **不做**（选项中未选该条 ⇒ 按"不加节流"实施；残余风险见 §5.2-R1） |
> | Q2 | 审计落点 | **(A) 不写唤醒注册表** |
> | Q3 | 是否可投给 skiff / 外部面 | **允许** |
...
| 3 | 「来**回应**」 | ✅ **已裁 = 单向投递**（「投递即可，不用管回应」）⇒ 本稿只做"把话递到"这一层，**不做**取回目标的答复。请求-响应**明确不做**（原 §7-Q1 备选 (b) 关闭） |
...
| # | 风险 | 为什么 wake 没有 | 建议护栏 |
|---|---|---|---|
| R1 | **无节流 ⇒ 乒乓**：A 发 B、B 的轮次里又发 A，无限往复 | wake 天然限速（必须未来 + 5min tick 粒度） | 🔴 **所有者 2026-09-16 裁：不做节流**（该项在 Q2 中未选）⇒ **本版无护栏**。**残余风险的兜底不靠机制而靠可观测性**：每一次 send 都会在**目标自己的会话日志**里留下一条 `[即时消息] 来自 <sender>` ⇒ 环形往复**肉眼可见**（不存在"静默烧额度"）。若将来要加，成本极低（同 sender→target 10s 冷却 ≈ 5 行），**不动接口** |
...
### 5.3 审计（**所有者 2026-09-16 裁：不写注册表**）

| 方案 | 做法 | 代价 |
|---|---|---|
| **(A) 不写** ✅ **已裁** | 回执同步给调用方；调用方把它记进自己的 SESSION.md | 无 schema 变更；**唤醒注册表保持"未来时刻表"的单一语义** |
| (B) 写终态条目 | `WakeEntry` 加 `kind?: 'wake' \| 'send'`；send 条目**一经写入即终态**（`delivered`/`missed`，**永不 `pending`**） | ⚠️ 需加"永不 pending"特例（否则 `pending` 条目会被调度器**每 tick 重试**，对一次性即时投递是错的），且要动 `asEntry` 校验 ⇒ **不采用** |
...
## 7. 裁决结果（2026-09-16 所有者）与实施前置

| # | 问题 | 备选 | **裁决** |
|---|---|---|---|
| **Q1** | 「用来**回应**」= 单向投递，还是请求-响应？ | (a) 单向 + 投递回执｜(b) 请求-响应（发送方阻塞等目标跑完并取回其答复） | ✅ **(a) 单向投递**。原话：「**投递即可，不用管回应**」。⇒ **(b) 关闭**（原倾向理由仍成立：D58 明文否过请求-响应；且 (b) 需宿主"等轮次结束 + 取输出"的能力，现在没有） |
| **Q2a** | 防乒乓节流 | 开（默认 10s）/ 不开 | ⛔ **不开**（该项未选）⇒ **v1 无节流**；残余风险与兜底见 §5.2-R1 |
| **Q2b** | 审计落点 | (A) 不写注册表｜(B) 写终态条目 | ✅ **(A) 不写** ⇒ 唤醒注册表**保持"未来时刻表"单一语义**，无 schema 变更 |
| **Q3** | 是否可投给 skiff / 外部面 | 允许 / 禁止 | ✅ **允许** |

⇒ **设计已确认（v0.2），无遗留决策点。** 下一步只剩**实施**，且实施**需 D14 的显式发版令**（ACC 代码改动）。

### 7.1 实施时的两个提醒（给自己）

1. **同批必改清单要现 grep 穷举**（§6 的表只是**已实测锚点**，不是闭集）—— 这是 §12.36 的教训（基线列 12 处，实际涟漪 387 处）。
2. **规格面（specs）要带一次修订**：`container_trajectory` 的动作数 6 → 7 是对外契约，属"标准"变更。
