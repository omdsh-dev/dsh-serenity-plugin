...
**Instant delivery**: `container_trajectory send-now` hands a message over **right now** — if the target is in memory it is injected immediately; if not, it is **cold-loaded first and then delivered** (equivalent to a direct wake, but **not waiting for the scheduler's 5-minute tick**). It returns a **synchronous receipt** (telling you whether it went live or cold-resume) and does **not** touch the registry (that table is only for "future instants").
...
- ⚠️ **The receipt only means "queued"**: it does **not** mean the target has run or replied. If the target is mid-round, the message takes effect at its **round boundary** (**it does not interrupt the current round**); and **one message per round boundary, first-in-first-out** — two messages queued together do **not** arrive in the same round. To confirm the target actually moved, read **its own `SESSION.md`** (a file-level criterion).
