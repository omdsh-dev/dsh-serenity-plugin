...
- **A trajectory can carry its own skills**: write `skills: [name, …]` in the top frontmatter of its `SESSION.md`, and every request **while that trajectory is bound** gets the full text of those skills injected (handy for pinning one domain's way of working onto a trajectory). ⚠️ This is "**kept supplied for as long as it is bound**", not "poured in once at `use`" — editing the frontmatter takes effect **immediately**, and it does not disappear on compaction. `create` merely creates: it **does not steal the current binding**, so a new trajectory must be **explicitly `use`d** before its skills are injected. Skill names come from workspace data and are **safety-checked before they are ever used to build a path**; a missing skill is reported as "missing" in the prompt rather than **silently skipped**. ⚠️ **Keep the top frontmatter intact when rewriting `SESSION.md`** — losing it silently revokes the declaration.
