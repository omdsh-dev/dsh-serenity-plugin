...
describe('v1.23.3 重建提醒：不做节流（每轮都催）+ 连续 3 轮升级强制语气（用户拍板）', () => {
  ...
  function captureWithPressure(sessionId = 's1'): {
    ...
    __setSimpleSourceForTest(() => ({ gatewayEnabled: false, rebuildEnabled: true, rebuildThresholdK: 8, skiffEnabled: false, skiffDebugPort: 3099, acpEnabled: false, acpHttpPort: 3100, publicAskEnabled: false, croEnabled: true }))
