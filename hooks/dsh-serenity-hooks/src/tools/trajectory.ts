...
// 🔴 2026-09-25（S142 工程化程序第 ④ 项·第 1 步）：`sanitizeSessionSummary` 与 `namingTitleFor`
// **已下沉到 L1 的 `trajectory-ops.ts`** —— 解 L1→L3 上行边 ＋ `rebuild.ts` ⇄ 本文件的依赖环
// （证据＝模块图 §3-6）。此处只保留**用**它们的门面（`renameDshSessionOnUse` 等）。
